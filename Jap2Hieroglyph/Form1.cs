...
namespace Jap2Hieroglyph
{
    public partial class Form1 : Form
    {
        ...
        readonly Dictionary<string, string> getHieroglyph = new()
        {
            {"あ", "\U0001313f" },
            {"い", "\U000131cc" },
            {"う", "\U00013171" },
        ...
        private void lang_jap_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                Debug.WriteLine(c);
                if (getHieroglyph.ContainsKey(c.ToString()))
                {
                    output += getHieroglyph[c.ToString()];
                }
                else
                {
                    output += c.ToString();
        ...
        private void lang_hiero_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                rest_of_len = lang_hiero.Text.Length - (i * 2);
                if (rest_of_len >= 6 && getHieroglyph.ContainsValue(lang_hiero.Text.Substring(i * 2, 6)))
                {
                    // 三つのコードポイントで構成されているヒエログリフを日本語に変換する
                    getHiero = lang_hiero.Text.Substring(i * 2, 6);
                    Debug.WriteLine("3: " + getHiero);
                    i += 2;
                }
                else if (rest_of_len >= 4 && getHieroglyph.ContainsValue(lang_hiero.Text.Substring(i * 2, 4)))
                {
                    // 二つのコードポイントで構成されているヒエログリフを日本語に変換する
                    getHiero = lang_hiero.Text.Substring(i * 2, 4);
                    Debug.WriteLine("2: " + getHiero);
                    i += 1; // 二つで一つと認識させる。
                }
                else
                {
                    // 一つのコードポイントで構成されているヒエログリフを日本語に変換する
                    getHiero = lang_hiero.Text.Substring(i * 2, 2);
                    Debug.WriteLine("1: " + getHiero);
                }

                if (getHieroglyph.ContainsValue(getHiero))
                {
                    output += getHieroglyph.First(x => x.Value.Equals(getHiero)).Key;
                }
                else
                {
                    output += getHiero;
        ...
        {
            _mediaPlayer.URL = @"music.mp3";
            _mediaPlayer.settings.volume = 75;
